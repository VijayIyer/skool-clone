...
export async function handleUserValidation (userObj:any, user_name: string, res: NextApiResponse) {
...
export async function handlePostContentRequiredValidation (title: string, content: string, category: string, user_id: string, user_name: string, res: NextApiResponse) {
...
export async function handlePostEmptyContentValidation (comments: string[], likes: string[], res: NextApiResponse) {
...
export async function handlePostPollValidation (poll: any[], res: NextApiResponse) {
    if (poll.length === 1) {
...
export async function handlePostGroupValidation (userGroups: string[], group: string, res: NextApiResponse) {
...
export async function handlePostAuthorValidation (foundObj:any, user_id: string, res: NextApiResponse) {
    if (!foundObj.isFound) {
...
export async function handlePostExistValidation (foundObj:any, res: NextApiResponse) {
    if (!foundObj.isFound) {
        const { message, status } = foundObj;
        await res.status(status).json({message});
        return;
